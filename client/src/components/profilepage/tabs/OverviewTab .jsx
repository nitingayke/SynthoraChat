    ...
    >
      ...
      <div className="bg-white dark:bg-[#161616] rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <InfoItem icon={Calendar} label="Joined" value={new Date(user.joinDate).toLocaleDateString()} />
          <InfoItem icon={Clock} label="Last Active" value={new Date(user.lastActive).toLocaleDateString()} />
        </div>
      </div>
        ...
        <div className="space-y-3">
          {user.credentials.map((cred, index) => (
            <div key={index * 0.5} className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              <div>
                <div className="font-semibold text-gray-900 dark:text-white">{cred.title}</div>
